# 
namespace :bittr_get_price do
  task :start, [] => :environment do |_cmd, args|
    puts "Run rake bittr_get_price:start"
...
class BittrPrice
  ...
  def save_price
    ...
    records = BittrPriceLog.where("pair_name = ? AND time_at <= ?", @pair, time_at).order(id: 'desc').limit(4)
  ...
  def find_pump(price_log)
    ...
    records = BittrPriceLog.where("pair_name = ? AND time_at <= ?", @pair, price_log.time_at).order(id: 'desc').limit(4)
